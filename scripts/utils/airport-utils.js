...
export function getValidAirportsForRegion(airports, region) {
  ...
  return (airports || []).filter((a) => {
    ...
    const sameCountry = !countryCodeParam || (a.countryCode || '').toUpperCase() === countryCodeParam;
    return hasDestinations && sameCountry;
  });
}

function getAdditionalAirportCodes(overridesMap, arrivalAirports) {
  const overrideValues = overridesMap && typeof overridesMap === 'object' ? Object.values(overridesMap) : [];
