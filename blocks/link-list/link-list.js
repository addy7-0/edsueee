  ...
  try {
    ...
    if (allPagesData.ok) {
      ...
      let filteredLinks = depthOneLinks.filter(
        (item) => !removeLinks?.some((excludedPath) => item?.pagePath === excludedPath)
...
export default async function decorate(block) {
  ...
  const buildListUsingType = promoteFirstChildIfExists(buildListUsingTypeRow)
    ?.textContent?.trim().toLowerCase()
    || 'fixedlist';
  const childPagesParentPage = childPagesParentPageRow;
  const childPagesOrderBy = promoteFirstChildIfExists(childPagesOrderByRow)?.textContent?.trim() || 'jcr:title';
  const childPagesSortOrder = promoteFirstChildIfExists(childPagesSortOrderRow)?.textContent?.trim() || 'ascending';
  const childPagesExcludeUrl = promoteFirstChildIfExists(childPagesExcludeUrlRow)?.textContent?.trim() || '';
  ...
  if (buildListUsingType === 'fixedlist') {
    [...linkListItems].forEach(async (row) => {
      ...
      const contentRow = promoteFirstChildIfExists(row) || row;
